...
```{ojs}
//| output: false
selected_dataset_name = selected_dataset ? selected_dataset.dataset : null;
selected_data = selected_dataset_name ? db.sql`select * from vizdata where dataset = ${selected_dataset_name}` : [];
kde = require('fast-kde');
distances_data = selected_dataset_name ? db.sql`select mahalanobis_distance_to_data from vizdata where dataset = ${selected_dataset_name} and part = 'train'` : [];
density_data = distances_data.length > 0 ? kde.density1d(distances_data.map(d => d.mahalanobis_distance_to_data), { bandwidth: 3, bins: 512, pad: 4 }) : null;
distances_query = selected_dataset_name ? db.sql`select mahalanobis_distance_to_data from vizdata where dataset = ${selected_dataset_name} and part = 'test'` : [];
density_query = distances_query.length > 0 ? kde.density1d(distances_query.map(d => d.mahalanobis_distance_to_data), { bandwidth: 3, bins: 512, pad: 4 }) : null;
...
Plot.plot({
  ...
  height: width/4,
  axis: null,
  color: {domain: ["train", "test"], range: ["#1f77b4", "#ff7f0e"]},
  marks: [
...
Plot.plot({
  ...
  marks: [
    ...(density_data ? [Plot.lineY(density_data.bandwidth(bw), {x: 'x', y: 'y', stroke: '#1f77b4', fill: '#1f77b4', fillOpacity: 0.5 })] : []),
    ...(density_query ? [Plot.lineY(density_query.bandwidth(bw), {x: 'x', y: 'y', stroke: '#ff7f0e', fill: '#ff7f0e', fillOpacity: 0.5 })] : []),
    Plot.ruleY([0])
  ]
})
```
